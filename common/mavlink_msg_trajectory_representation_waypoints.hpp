 ...
 *
 * WORK IN PROGRESS! DO NOT DEPLOY! Describe a trajectory using an array of up-to 5 waypoints in the local frame.
 */
struct TRAJECTORY_REPRESENTATION_WAYPOINTS : mavlink::Message {
    ...
    uint64_t time_usec; /*< Timestamp (since system boot or since UNIX epoch). */
    uint8_t valid_points; /*< Number of valid points (up-to 5 waypoints are possible) */
    std::array<float, 5> pos_x; /*< X-coordinate of waypoint, set to NaN if not being used */
    std::array<float, 5> pos_y; /*< Y-coordinate of waypoint, set to NaN if not being used */
    std::array<float, 5> pos_z; /*< Z-coordinate of waypoint, set to NaN if not being used */
    std::array<float, 5> vel_x; /*< X-velocity of waypoint, set to NaN if not being used */
    std::array<float, 5> vel_y; /*< Y-velocity of waypoint, set to NaN if not being used */
    std::array<float, 5> vel_z; /*< Z-velocity of waypoint, set to NaN if not being used */
    std::array<float, 5> acc_x; /*< X-acceleration of waypoint, set to NaN if not being used */
    std::array<float, 5> acc_y; /*< Y-acceleration of waypoint, set to NaN if not being used */
    std::array<float, 5> acc_z; /*< Z-acceleration of waypoint, set to NaN if not being used */
    std::array<float, 5> pos_yaw; /*< Yaw angle, set to NaN if not being used */
    std::array<float, 5> vel_yaw; /*< Yaw rate, set to NaN if not being used */
